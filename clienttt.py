...
def checkHash(message, hashValue):                                  #function for checking hash value matches hashed message
    if hash(message)== int(hashValue):
        return True
    else: 
        return False
...
def receiving():                                                    #function that is turned into thread and receives messages from the server
    while True:
        ...
        actMessage = messageArr[0].replace('\t','')                 #removes tabbing in front of message, in order to compare with the right hash function
